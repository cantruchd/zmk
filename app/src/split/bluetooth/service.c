...
// ← WPM + TOTAL KEYSTROKES HANDLERS
static uint8_t wpm_value = 0;
static uint32_t total_keystrokes = 0;  // ← THÊM: Lưu total keystrokes

static void split_svc_wpm_received_callback(struct k_work *work) {
    LOG_INF("Peripheral received from central: WPM=%d, Total keystrokes=%d", 
            wpm_value, total_keystrokes);
    
    // ← THÊM: Raise event với cả 2 giá trị
    raise_zmk_split_wpm_state_changed(
        (struct zmk_split_wpm_state_changed){
            .wpm = wpm_value,
            .total_keystrokes = total_keystrokes  // ← THÊM
        }
    );
}
...
// ← THAY ĐỔI: Nhận 5 bytes (1 WPM + 4 total_keystrokes)
static ssize_t split_svc_receive_wpm(struct bt_conn *conn, const struct bt_gatt_attr *attr,
                                     const void *buf, uint16_t len, uint16_t offset,
                                     uint8_t flags) {
    // ← THAY ĐỔI: Kiểm tra length = 5 bytes
    if (len != 5) {
        LOG_ERR("Invalid WPM data length: %d (expected 5)", len);
    ...
    // ← THAY ĐỔI: Unpack 5 bytes
    const uint8_t *data = (const uint8_t *)buf;
    
    wpm_value = data[0];  // Byte 0: WPM
    memcpy(&total_keystrokes, &data[1], 4);  // Bytes 1-4: total_keystrokes (uint32_t)
    
    LOG_DBG("Received WPM write: WPM=%d, Total keystrokes=%d", wpm_value, total_keystrokes);
